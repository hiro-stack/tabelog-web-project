...
　(1)特徴→財力、体力、人数、好みなど<br>
　(2)状況→誕生日や接待、騒ぎたい・静かにしたい、できるだけ早くお店に行きたいなど<br>

 これらの「特徴」と「状況」は、グループごとに何を重視するか<br>
 （=意思決定の軸）を変化させる要因である。<br>
...
これらを整理すると、  
特徴と状況のパターンを考えてみると多くのケースにおいて集団での意思決定を左右している要素は、<br>
- 評価  
- 距離  
- 価格  
- 食べ物の好み（およびその好みを持つ人数）<br>
  の4項目に集約できると仮定できる。<br>

さらに、現在のシステムで取得可能なデータやアルゴリズムの制約を踏まえても、この4項目を軸とするモデルは現実的かつ実装可能であると判断した。
...
- **主に使用したライブラリ**：Selenium, webdriver, pandas, folium
...
#### シチュエーション例：
自分の立場としては今からサークルの幹事としてグループに合ったお店を調べるという立場を想定。グループの状況として、時間帯はお昼、現在地は東京電機大学。若い子が多く、現在地から探索するエリアは北千住駅。移動距離に関しては遠くまで行ける体力がありそう。しかし金銭的な余裕がない子が多い印象。ただ話すことが目的なので食べ物のおいしさとかはある程度無視できる。今回は誰かの誕生日とかではなくて多数決で食べたいものを決める。合計5人で食べたいものが[ラーメン、ラーメン、そば、居酒屋、居酒屋]である。最大で支払える額が大体5000円、歩く距離の上限が徒歩で20分。という状況を仮定。
...
### トップページ
![トップページ](./screenshot_images/入力フォーム1.png)
![トップページ](./screenshot_images/入力フォーム2.png)

---

### 入力確認ページ
![入力確認](./screenshot_images/確認画面.png)

---

### 処理中のDockerの様子
![Dockerの動き](./screenshot_images/backendの処理.png)

---

### 検索結果ページ
![検索結果ページ1](./screenshot_images/処理結果1.png)
![検索結果ページ2](./screenshot_images/処理結果2.png)
![地図の表示](./screenshot_images/地図表示.png)

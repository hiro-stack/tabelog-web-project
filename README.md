...
普段利用していた飲食店の推薦型のアプリで、グループごとにお店を選ぶ際、評価の高い順にソートされた結果が必ずしも最適ではないことに疑問を持ちました。実際には、移動距離が遠すぎたり、学生では手が届かない価格帯の店が上位に表示されるなど、グループの状況やニーズが反映されていない点が課題だと感じました。
システムを利用するグループの状態やシーンを細かく分析し、「どんな要因がお店選びの決め手になるのか」を洗い出しました。その過程で、グループの状況によって優先される項目が変化することに気づき、距離・価格・評価などの指標に重み付けを行い、状況に応じて動的に評価が変わるロジックとしてシステムに組み込みました。

スクレイピングをした理由
→「ぐるなび」のようなAPIの情報には「星評価食」の項目がなかったりしました。
より効率的かつ公平な意思決定をサポートするためのロジックに必要なデータをそろえるためにはtabelogのデータをスクレイピングが必要だと判断しました。

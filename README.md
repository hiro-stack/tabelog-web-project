...
そこで、利用するグループの状態やシーンを分析し、「どんな要因がお店選びの決め手になるか」を洗い出しました。
「距離」「価格」「評価」などの要素に重み付けを行い、状況に応じて動的に評価が変化するロジックを開発。さらに、特定の誰かの誕生日会であればその人の好みを優先する、シンプルな多数決が求められる場面では民主的な決定を反映するなど、意思決定のスタイルに応じた柔軟な評価方法も組み込んでいます。
...
バックエンド側で `media/result_data.csv`, `result_map.html` を生成できたが、  
フロントで取得しようとすると空になっていた。
...
- **① media/ にデータが格納されていない？**  
  → media/ で両ファイルが生成されていることを確認。

- **② バックエンドのフロントへの送出に問題？**  
  → 要確認。公開されているはずの `/media/` にアクセスしたら「見つからない」とエラー。

- **③ Docker の設定に問題？**  
  → 他のデータは取得できていたため、可能性は低いと判断。

- **④ フロントエンドの受け取り処理に問題？**  
  → 他のデータは正常に取得。media だけが失敗している可能性は低い。

- **⑤ 表示部分の問題？**  
  → 要確認。実際にファイルが「見つからない」と 404 エラー。

---

#### (2) 原因の特定

- 問題は「ファイル送出の部分」にあると判断。

- ブラウザで  
  `http://localhost:8000/media/result_data.csv`  
  にアクセスしようとしたところ、

  `http://localhost:8000/config/media/result_data.csv`  
  に**リダイレクトされてしまっていた。**
...
- つまり、**自動変換のURLに問題がある**と特定。

---

#### (3) エラーの解消

- Django の設定で `BASE_URL` が `http://localhost:8000/config` になっていたため、  
  メディアファイルへのアクセスが意図しないパスに変換されていた。

- **これを `http://localhost:8000` に修正**することで、正常にメディアファイルが表示されるようになった。

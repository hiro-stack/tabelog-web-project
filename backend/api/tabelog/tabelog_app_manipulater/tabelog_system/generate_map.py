...
class Mapping:
    def __init__(self, current_location):
        self.current_location = current_location
         ...
         # このファイル自身（.py）のディレクトリを取得
        base_dir = Path(__file__).resolve().parent

        # このファイル基準で相対パスを安全に結合
        self.input_csv_path = base_dir / "data" / "result_data" / "result_data.csv"
        self.output_html_path = base_dir / "data" / "result_data" / "mapping.html"

        # mappingするときの色の設定
        self.colors = ["red", "orange", "lightblue", "blue"]

        # 区切りポイント（1-3, 4-6, 7-10）
        self.segments = [3, 5, 10]

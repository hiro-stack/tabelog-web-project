...
class Analysts:
    def __init__(self, time_is, max_price, max_minutes, weight):
        ...
        # 入力・出力フォルダをこのファイル基準のパスにする
        self.input_folder = os.path.join(current_path, "data", "maked_data")
        self.output_folder = os.path.join(current_path, "data", "result_data")
    ...
    def data_filterring_time(self):
        self.result_df = self.result_df[
            self.result_df["現在地点からの徒歩(分)"] <= self.max_minutes
        ]
    ...
    def data_reciprocal_price(self):
        self.result_df["価格"] = self.result_df["価格"].apply(
            lambda x: 1 / x if x != 0 else 0
        )

    def data_reciprocal_distance(self):
        self.result_df["現在地点からの徒歩(分)"] = self.result_df[
            "現在地点からの徒歩(分)"
...
if __name__ == "__main__":
    waight = {"distance": 1.1, "budget": 1.2, "evaluate": 1.3}

...
import re
from .calculate_location import Geo
...
class DataMaker:
    ...
    def voice_force_manipulate(self):
        ...
        self.combined_df["声の大きさ"] = self.combined_df["項目"].map(
            self.voice_results
        )
    ...
    def geo_manipulate(self):
        ...
        self.combined_df["現在地点からの徒歩(分)"] = self.combined_df[
            "現在地からの距離(km)"
        ].apply(lambda x: (x / self.walk_speed_kmh) * 60 if x is not None else None)
    ...
    def extract_average_price(self, price_str):
        ...
        prices = list(
            map(
                ...
                re.findall(r"\d{1,3}(?:,\d{3})*", price_str),
            )
        )
        if prices:
    ...
    def output_data_file(self):
        ...
        output_path = os.path.join(
            current_path, self.output_folder_path, self.output_csv_name
        )
...
if __name__ == "__main__":
    ...
    voice_force = {"スペイン料理": [3], "刀削麺": [5, 4, 3]}

    alpha_value = 0.5

    app = DataMaker(current_location, voice_force, alpha_value)
    app.main()
